...
#
# Notes:
# A) Code to generate Supplemental Figure 1 in the manuscript
# B) Male Lung Cancer Mortality Rate by Male Current Smoking Prevalence
...
# Male Current Smoking Prevalence
## Preparation
CSM <- Lee[stats::complete.cases(Lee$CSM_Pct_1997_2003), ]
bks <- as.vector(quantile(Lee$CSM_Pct_1997_2003, probs = seq(0, 1, 0.1), na.rm = TRUE))
bks[1] <- 0 # set lowest limit to zero
CSM$at <- cut(CSM$CSM_Pct_1997_2003, bks, dig.lab = 20)
levels(CSM$at) <- levels(CSM$at)[gtools::mixedorder(levels(CSM$at), decreasing = TRUE)]
levels(CSM$at) <- stringr::str_replace(levels(CSM$at), ",", " - ") %>%
  stringr::str_replace(., "[(]", "") %>%
  stringr::str_replace(., "[]]", "")
tmp <- sprintf("%.1f", as.numeric(unlist(strsplit(levels(CSM$at), "-"))))
levels(CSM$at) <- paste(tmp[seq(1, 19, 2)], tmp[seq(2, 20, 2)], sep = " - ")
levels(CSM$at) <- c(levels(CSM$at), "Suppressed")
levels(CSM$at) <- levels(CSM$at)[gtools::mixedorder(levels(CSM$at), decreasing = FALSE)]
CSM$at <- relevel(CSM$at, "Suppressed")
levels(CSM$at) <- c("Suppressed", "5.5 - 19.6", "19.7 - 22.3", "22.4 - 24.3", "24.4 - 26.0", "26.1 - 27.5",
                           "27.6 - 28.9", "29.0 - 30.2", "30.3 - 31.8", "31.9 - 34.2", "34.3 - 49.3")
## Plotting
PCSM <- ggplot2::ggplot() + 
  ggplot2::geom_sf(data = Lee,
                   fill = "white",
                   color = "black") +
  ggplot2::geom_sf(data = CSM,
                   ggplot2::aes(fill = at),
                   color = "black") +
  ggplot2::geom_sf(data = proj_l48,
                   fill = "transparent",
...
# Lee's L for Male Lung Cancer Mortality Rate by Male Current Smoking Prevalence
LCSM <- ggplot2::ggplot() + 
  ggplot2::geom_sf(data = Lee,
                   ggplot2::aes(fill = LCSM_LeeFDR),
                   color = "black") +
  ggplot2::geom_sf(data = proj_l48,
                   fill = "transparent",
                   color = "black",
...
# Combine for Supplemental Figure 1
BCSM <- cowplot::align_plots(CMRM, PCSM, LCSM, align = "hv", axis = "tblr")
gg_inset_map <- cowplot::ggdraw() +
  cowplot::draw_plot(BCSM[[1]], y =  0.33, scale = 0.9) +
  cowplot::draw_plot(BCSM[[2]], y =  0.00, scale = 0.9) +
  cowplot::draw_plot(BCSM[[3]], y = -0.33, scale = 0.9)

# Print
grDevices::png(file = "figures/supplemental1.png", width = 850*f, height = 1100*f)
print(gg_inset_map)
grDevices::dev.off()
